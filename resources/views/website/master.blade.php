...
<head>
    ...
    <title>@yield('title') | CityMall - No 1 Ecommerce Website</title>
...
<body>
                            ...
                            @if (Session::get('customer_id'))
                                <div class="user">
                                    <nav class="navbar navbar-expand-md">
                                        <i class="lni lni-user mb-1"></i>
                                        <ul class="navbar-nav">
                                            <li>
                                                <a href="javascript:void(0);" class="nav-link text-white dropdown-toggle" data-bs-toggle="dropdown">{{Session::get('customer_name')}}</a>
                                                <ul class="dropdown-menu">
                                                    <li><a href="{{route('my-dashboard')}}" class="nav-link text-dark">Dashboard</a></li>
                                                    <li><a href="{{route('my-profile')}}" class="nav-link text-dark">Profile</a></li>
                                                    <li><a href="" class="nav-link text-dark">Orders</a></li>
                                                    <a href="{{route('customer-logout')}}" class="nav-link text-dark btn btn-sm bg-light">Logout</a>
                                                </ul>
                                            </li>
                                        </ul>
                                    </nav>
                                ...
                                <div class="cart-items">
                                    ...
                                    <div class="shopping-item">
                                        ...
                                        <ul class="shopping-list">
                                            ...
                                            <li>
                                                <a href="{{route('remove-cart-item', ['id' => $cartProduct->id])}}" onclick="return(confirm('Confirm to delete ?'))" class="remove" title="Remove this item"><i
                                                        class="lni lni-close"></i></a>
                                                <div class="cart-img-head">
                                                    <a class="cart-img" href="{{asset($cartProduct->attributes->image)}}" target="__blank"><img
                                                            src="{{asset($cartProduct->attributes->image)}}" alt="#"></a>
                                                </div>
                                                <div class="content">
                                                    <h4><a href="{{route('product-details-info', ['id' =>$cartProduct->id])}} ">
                                                            {{$cartProduct->name}}</a></h4>
                                                    <p class="quantity">{{$cartProduct->quantity}}x - <span class="amount"><span>&#2547;</span>{{$cartProduct->price}}</span></p>
                                                </div>
                                            </li>
                                            <?php $totalPrice += $cartProduct->quantity * $cartProduct->price ?>
                                            @endforeach
                                        </ul>
                                        <div class="bottom">
                                            <div class="total">
                                                <span>Total</span>
                                                <span class="total-amount"><span>&#2547;</span>{{number_format($totalPrice)}}</span>
                                            </div>
                                            <div class="button">
                                            @if (count($cartProducts) > 0)
                                                <a href="{{route('checkout')}}" class="btn animate">Checkout</a>
                                            @else
                                                <a href="javascript:void(0);" class="btn animate bg-light text-danger" style="cursor: auto;">Checkout</a>
                                            @endif
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <ul class="sub-menu collapse" id="submenu-1-3">
                                            @if (count($brands) > 0)
                                                @foreach ($brands as $brand)
                                                    <li class="nav-item"><a href="{{route('brand-products', ['id' => $brand->id])}}">{{$brand->name}}</a></li>
                                                @endforeach
                                            @else
    ...
    <script src="{{asset('/')}}website/assets/js/jquery-3.6.0.js"></script>
...
<script>
    function user_dashboard(){
        // return(confirm('Hey Dude'));
        console.log('Hey.clicked on dashboard');
        $.ajax({
            type:'GET',
